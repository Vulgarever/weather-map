...
var WeatherMap = (function () {
    ...
    var METEO_CONFIG = {
        ...
        windSpeed: {
            ...
            max: 30,
            colors: [
                { val: 0, r: 173, g: 216, b: 230, a: 0, hex: "rgba(173,216,230,0)" },
        ...
        cloud: {
            ...
            max: 100,
            colors: [
                { val: 0, r: 255, g: 255, b: 255, a: 0, hex: "rgba(255,255,255,0)" },
    ...
    var SnowParticleLayer = L.Layer.extend({
        ...
        _buildMask: function () {
            ...
            var cols = 160, rows = Math.ceil(cols * ((north - south) / (east - west)));
    ...
    function bindMapPopup() {
        map.on("click", function (e) {
            ...
            function check(v) { return (v !== undefined && v !== 9999 && v !== 999999 && v !== -9999 && v !== -999); }
            ...
            if (station) {
                if (activeType === "temp" && check(station[19]))
                    rows.push('<div class="popup-row">🌡️ <b>气温:</b> ' + station[19] + "°C</div>");
                else if (activeType === "rain" && check(station[12]))
                    rows.push('<div class="popup-row">💧 <b>降水:</b> ' + station[12] + " mm</div>");
                else if (activeType === "snow" && check(station[12]))
                    rows.push('<div class="popup-row">❄️ <b>降雪量:</b> ' + station[12] + " mm</div>");
                else if (activeType === "wind" || activeType === "windSpeed") {
                    if (check(station[5])) rows.push('<div class="popup-row">🌪 <b>实况风速:</b> ' + station[5] + " m/s</div>");
                ...
                } else if ((activeType === "pressure" || activeType === "isobar") && check(station[8]))
                    rows.push('<div class="popup-row">⏱ <b>气压:</b> ' + station[8] + " hPa</div>");
                else if (activeType === "humidity" && check(station[9]))
                    rows.push('<div class="popup-row">💧 <b>相对湿度:</b> ' + station[9] + " %</div>");
                else if (activeType === "radiation" && check(station[10]))
                    rows.push('<div class="popup-row">☀️ <b>太阳辐射:</b> ' + station[10] + " W/m²</div>");
                else if (activeType === "cloud" && check(station[11]))
                    rows.push('<div class="popup-row">☁️ <b>总云量:</b> ' + station[11] + " %</div>");
                else if (activeType == null && isOn("weather") && station[4] != null) {
            ...
            var html = '<div class="popup-title">📍 ' + (place ? place + "  ·  " : "") + lat.toFixed(2) + "°N, " + lng.toFixed(2) + "°E</div>" + rows.join("");
    ...
    function fetchAllRealData(callbacks) {
        callbacks = callbacks || {};
        return Promise.all([
            fetch(APIs.CMISS).then(function (r) { return r.json(); }).catch(function () { return null; }),
            fetch(APIs.GEO_BOUNDS).then(function (r) { return r.json(); }).catch(function () { return null; }),
        ])
            .then(function (results) {
                ...
                if (resGeo && resGeo.features) {
                    ...
                    if (resGeo.features.length > 0) {
                        map.fitBounds(L.geoJSON(resGeo).getBounds(), { padding: [60, 60], animate: false });
            ...
            .catch(function (err) {
                if (typeof callbacks.onError === "function") callbacks.onError("数据拉取失败！");
                throw err;
            });
    }
    ...
    function refreshCmissData(callbacks) {
        callbacks = callbacks || {};
        return fetch(APIs.CMISS).then(function (r) { return r.json(); }).catch(function () { return null; })
            .then(function (cmiss) {
                if (!cmiss) {
                    if (typeof callbacks.onError === "function") callbacks.onError("cmiss 数据加载失败");
                    return api;
                }
